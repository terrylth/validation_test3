...
"""
flawed_model_script.py

Interview practice scenario:
Generic escalation classification model with tabular + text features.

Candidate task:
Review this as a model validator. You are NOT expected to catch everything.
Focus on:
- data leakage
...
- whether model performance is believable
"""
...
class EscalationDataBuilder:
    ...
    def build(self):
        ...
        # Business process variable that is only populated after operational review starts.
        # It looks innocent but may not be available at prediction time.
        ...
        # This score is calculated by an operations team after reviewing the case.
        # It is not available when the model is supposed to score a new incoming case.
        ...
        # Create missingness that is not random.
...
class EscalationFeatureEngineer(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.global_segment_rate_ = None
        self.global_channel_rate_ = None

    def fit(self, X, y=None):
        tmp = X.copy()

        # FLAW: target encoding is fitted using all data when called before split.
        # Even worse, fit accepts y but uses target if it exists in X.
        if "escalated" in tmp.columns:
            self.global_segment_rate_ = tmp.groupby("customer_segment")["escalated"].mean().to_dict()
            self.global_channel_rate_ = tmp.groupby("channel")["escalated"].mean().to_dict()
        elif y is not None:
            tmp["escalated"] = y
            self.global_segment_rate_ = tmp.groupby("customer_segment")["escalated"].mean().to_dict()
            self.global_channel_rate_ = tmp.groupby("channel")["escalated"].mean().to_dict()
    ...
    def transform(self, X):
        ...
        # FLAW: these are post-event / post-review variables.
        ...
        # FLAW: target encodings may have been fitted globally.
        df["segment_escalation_rate"] = df["customer_segment"].map(self.global_segment_rate_).fillna(0)
        df["channel_escalation_rate"] = df["channel"].map(self.global_channel_rate_).fillna(0)

        # FLAW: text feature directly searches for escalation-related words.
        # This may or may not be valid depending on prediction timing and whether the text is raw customer text or analyst-added notes.
...
class EscalationModelTrainer:
    ...
    def prepare_data(self, df):
        # FLAW: feature engineering is fit on the full dataset before train/test split.
    ...
    def split_data(self, X, y):
        # FLAW: random split for event-time data. If escalation process changes over time,
        # this gives an overly optimistic result.
    ...
    def build_pipeline(self):
        ...
        preprocessor = ColumnTransformer(
            transformers=[
                ...
                ("text", TfidfVectorizer(max_features=50, ngram_range=(1, 2)), text_feature)
            ],
            remainder="drop"
        )
    ...
    def cross_validate(self, X, y):
        ...
        # FLAW: plain KFold shuffles observations and ignores time.
        # Also uses X that already contains globally engineered leakage features.
